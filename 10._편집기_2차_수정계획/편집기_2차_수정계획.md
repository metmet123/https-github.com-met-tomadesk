...
> 2026-09-19 정리: 이 문서는 당시 설계 이력이다. Q6~Q8은 후속 구현에 반영되어 재승인 대기가 아니다. 휠 허용 시안은 최신 값 입력 오조작 방지 정책으로 대체한다. 공통 D-Day 규칙은 7일 이내 주황, 오늘/지난 기한 빨강이다. 최신 상태/미확인은 `../1._PROJECT_PLAN.md` 상단과 `../3._TODO.md`를 따른다.
...
4. **크기 칸** — 화살표를 없앤 44px 숫자 칸. 직접 입력·↑↓키와 최신 휠 오조작 방지 정책을 기준으로 확인한다.
5. **샘플 묶음** — 테두리를 공유한 한 묶음, 흰색 서식은 글자에 회색 외곽선, `⚙`는 단색 아이콘
6. **카테고리 칩** — 제목 칸(104px 고정) 바로 옆 `● 미지정 ▾`. 위쪽 한 줄 삭제(Q9)
7. **D-Day 칩 값** — `📌 D+16`, 7일 이내 주황, 오늘·지난 기한 빨강. 검은 배지 삭제(Q9·Q10)
...
| `editor.py` `_refresh_property_chips()` D-Day 칩 | 값이 있으면 `📌 D+16`처럼 **남은 날만**(기한 이름 제외), 툴팁은 `deadline_badge_text()` 전체. 색과 임박 판정은 공통 `deadline_urgency` 계약(7일 이내 주황, 오늘·지난 기한 빨강)을 따른다 |
...
- D-Day 설정 시 칩 글자 `📌 D+16`, 제목 줄에 검은 배지 없음, 7일 이내 주황, 오늘·지남 빨강
...
| 파일 · 위치 | 변경 |
|---|---|
| `text_format_toolbar.py` `size_box` | `setButtonSymbols(QAbstractSpinBox.ButtonSymbols.NoButtons)`, `setFixedWidth(44)`, 가운데 정렬. 직접 입력·↑↓키를 검증하고 휠은 최신 오조작 방지 정책을 확인 |
...
- [ ] 크기 칸 직접 입력·↑↓키 조절과 휠 오조작 방지 정책을 컨트롤별로 확인
...
Q6~Q8은 후속 구현에 반영되었다. 위 대안은 당시 결정 이력이며 현재 승인 질문이 아니다. 휠 동작은 문서 상단의 최신 정책을 적용한다.
