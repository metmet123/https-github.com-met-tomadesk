"""Memo usability scenarios on an isolated temporary database.

Run with ``python tools/memo_usability_test.py [S1 S2 ...]``. Set
``TOMADESK_USABILITY_OUT`` to retain the JSON report and Qt grabs for review.
"""
import json, os, sys, tempfile, traceback, time, re
...
_temporary_output = None if os.environ.get('TOMADESK_USABILITY_OUT') else tempfile.TemporaryDirectory(prefix='tomadesk_usability_')
OUT = Path(os.environ['TOMADESK_USABILITY_OUT']) if _temporary_output is None else Path(_temporary_output.name)
...
def s1():
    ...
    selector = 'QPushButton#memoCategoryFilterChip:checked'
    record('S1 목록', '필터 칩 선택(:checked) 스타일 정의', '카테고리 칩 전용 checked 규칙 존재', selector in ss, selector in ss)
ONLY=set(sys.argv[1:])
_run=run
def run(name, fn):
...
def s2():
    ...
    record('S2 머리영역', '서식 펼침 시 본문 높이 비율', '>=70%', f'본문 {body_h}px / 편집 영역 {total}px = {body_h*100//max(1,total)}%', body_h / max(1, total) >= 0.7, s1_)
    click(ed.format_expand_button)
    # The persistent format chip toggles the drawer; the gear owns settings.
    click(chips.buttons['format'])
    fmt_open = ed.format_panel.isVisible()
    s = shot_widget(win, 'S2_format_chip')
    record('S2 머리영역', "'서식' 칩 → 서식 도구", '서랍 열림', fmt_open, fmt_open, s)
    if fmt_open: click(chips.buttons['format'])
    # The hotkey chip edits this memo's global hotkey in the property drawer.
    click(chips.buttons['hotkey'])
    page = ed.property_panel.active_page
    s = shot_widget(win, 'S2_hotkey_chip')
    record('S2 머리영역', "'단축키' 칩 → 이 메모 전역 단축키", 'hotkey 페이지 열림', page, page == 'hotkey', s)
    click(chips.buttons['hotkey'])
    # 툴바 단축키 버튼
...
def s3():
    ...
    visible_ids = [int(it.data(1, Qt.ItemDataRole.UserRole)) for it in lp._note_items()]
    record('S3 카테고리', "'개발' 필터", "개발 카테고리만", shown,
           bool(visible_ids) and all(store.note(nid)['category_id'] == cats['개발'] for nid in visible_ids), s)
    record('S3 카테고리', '필터 결과에서 하위 메모 표시', '상속된 하위 메모 2개 표시', shown,
           c1 in visible_ids and c2 in visible_ids, s)
...
def s4():
    ...
    record('S4 보기·정렬', '일괄 지정 후 체크 상태', '모두 해제', still, not any(still))
    for nid in (a, b):
        it = lp._item_for(nid)
...
def s8():
    ...
    MENU_LOG.clear()
    point_cursor = QTextCursor(body.document()); point_cursor.setPosition(i + 2)
    QTest.mouseClick(body.viewport(), Qt.MouseButton.LeftButton, pos=body.cursorRect(point_cursor).center()); pump()
    popover_actions = MENU_LOG[-1] if MENU_LOG else []
    record('S8 주석', '본문 주석 클릭 → 수정·삭제 메뉴', '주석 내용과 수정·삭제', popover_actions,
           any('여기 다시 확인' in x for x in popover_actions)
           and any('수정' in x for x in popover_actions)
           and any('삭제' in x for x in popover_actions))
    QTest.keyClick(body, Qt.Key.Key_Z, Qt.KeyboardModifier.ControlModifier); pump()
    record('S8 주석', '본문 Ctrl+Z로 주석 추가 취소', '주석 0개', len(store.memo_data.annotations(nid)),
           len(store.memo_data.annotations(nid)) == 0)
    QTest.keyClick(body, Qt.Key.Key_Y, Qt.KeyboardModifier.ControlModifier); pump()
    record('S8 주석', '본문 Redo로 주석 복원', '주석 1개', len(store.memo_data.annotations(nid)),
           len(store.memo_data.annotations(nid)) == 1)
    click(ed.property_chips.buttons['other'])
...
def s10():
    ...
    txt = ' '.join(panel.summary.deadline_list.item(i).text()
                   for i in range(panel.summary.deadline_list.count()))
...
panel.shutdown(); win.close(); app.processEvents(); store.close()
...
app.quit()
if _temporary_output is not None:
    _temporary_output.cleanup()
# Some Qt popup objects keep the interpreter in finalization on Windows after
# every assertion and resource cleanup has finished. This isolated subprocess
# must still return a truthful status to the regression runner.
status = int(any(not row['ok'] for row in RESULTS) or bool(EXC))
