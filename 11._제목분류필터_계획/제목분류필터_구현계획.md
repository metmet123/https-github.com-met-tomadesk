...
| ③ 값 없음 | `☆` (▾ 없음) | `[ ]` (▾ 없음) | 점선 테두리 + 흐린 글자 (`empty="true"`) | **설명 말풍선** |
...
- 완료 기준: 세 상태 글자·속성, 동률 대표값, 말풍선 두 종류 문구, 96px 말줄임.
...
| 같은 파일 `test_category_chips_unassigned_filter_color_and_compact_cells` | D-4로 `미지정`이 기본 칩이 아님 | `미지정` 버튼을 `category_filter_buttons`가 아니라 더보기 메뉴 action에서 찾거나, 테스트 시작 시 `set_pinned_category_keys(store, [업무, "none"])` |
