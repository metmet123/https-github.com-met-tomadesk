...
| D-6 | 기호·머리말 버튼은 **가장 많이 쓴 값을 대표로** 보여 준다. 값이 없으면 흐린 **기호 / 머리말 텍스트**와 점선 테두리로 보이고, 누르면 **기능 설명 말풍선** | 2026-09-17 글자 확정 기준. 빈 아이콘 시안은 폐기 |
...
| 버튼 | `test_three_button_states` | ① `✨ 5 ▾` ② `✨ ▾`+checked ③ 흐린 `기호`·`머리말`+`empty` 속성, 점선 테두리, `▾` 없음 |
...
- [ ] 값이 없는 버튼의 `기호`·`머리말` 텍스트가 잘리지 않고 점선 테두리·흐린 색·`▾` 없음으로 표시됨
